// script.js - VERSÃO COM CORREÇÃO DE ATUALIZAÇÃO DA TABELA PÓS-REGISTRO
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * CORREÇÃO PRINCIPAL:
   * Esta função agora busca os dados e, em vez de apenas renderizar a tabela,
   * ela chama aplicarFiltros() para garantir que a visão do usuário seja consistente.
   */
  const carregarRegistros = async () => {
    try {
      const response = await fetch('/registros');
      todosOsRegistros = await response.json();
      aplicarFiltros(); // ALTERADO: Chama aplicarFiltros() para redesenhar a tabela respeitando os filtros.
    } catch (error) {
  ...
  if (gerarBtn) {
    gerarBtn.addEventListener('click', async () => {
      ...
      // Adicionado validação para campos de combo vazios
      if (!projeto || !tipoObra || !tipoProjeto || !tipoDoc || !disciplina) {
        alert('Por favor, selecione uma opção em todos os campos de lista.');
        return;
      }
      ...
      // MELHORIA: Adicionado tratamento de erro ao salvar
      try {
        const response = await fetch('/registro', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(dadosParaSalvar) });
        if (!response.ok) {
          throw new Error('Falha ao salvar o registro no servidor.');
        }
        ...
        carregarRegistros(); // Atualiza a lista e reaplica os filtros
      } catch (error) {
